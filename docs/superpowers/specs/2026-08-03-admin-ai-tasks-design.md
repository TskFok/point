...
- ~~不维护本地英文词库；下一词由 AI 按字母序续写。~~ 2026-08-04 起改为从数据库英文词库 `entry` 表取词（见 `2026-08-04-ai-task-entry-word-pos-design.md`）；词库数据的导入与维护由外部流程负责，本系统只读。
...
- `explanation`：中文；须含题干整句译文 + 词义/考点说明。详见 `2026-08-03-ai-question-explanation-include-stem-translation-design.md`。
- `basePoints` / `optionCount`：取自任务配置；`isActive` 默认 `true`。
- **词表来源（2026-08-04 起）**：每轮先从英文词库 `entry` 表取 `lastWord` 游标之后按字母序的 `questionCount` 个去重单词（聚合各词全部 `pos` 词性），prompt 中给出词表与词性，AI 只对这些词出题并按词性造句；返回的 `word` 必须在本批词表内且不重复，否则跳过该题。详见 `2026-08-04-ai-task-entry-word-pos-design.md`（取代原「密推进」规则 `2026-08-03-ai-task-dense-word-progression-design.md`）。
- AI 返回严格 JSON 数组；每项至少包含：`word`（本题锚定的英文单词）、`stem`、`explanation`、`options`（`label`/`content`/`isCorrect`）。`word` 用于校验词表归属与更新游标，**不**单独落库字段。入库前校验：stem 须含 `word` 且不得挖空，否则跳过该题。
- 游标：`lastWord` 为空 → 从词库字母序最前开始；成功写入至少 1 题后，将任务 `lastWord` 更新为本次成功题目中按字母序最大的 `word`（规范化小写），保证跨轮**不重复出题**。
...
3. 启动步骤（cron / manual 共用）：
   ...
   2. 记录 `lastWordBefore`。
   3. 解密模型密钥；从 `entry` 表取 `lastWord` 之后的 `questionCount` 个单词（含词性；词库取尽 → `FAILED`）；调用 OpenAI 兼容 `chat/completions`（超时 **60 秒**；单测 mock）。
   4. Prompt 要求：对给定词表逐词出题（附词性，例句须按该词性使用目标词）；每题带 `word`；题干为含 `word` 的完整英文例句并点名考查该词（禁止挖空）、选项中文词义、`optionCount` 个选项、恰一正确项、中文解析（须含整句题干译文 + 词性 + 词义说明）；**严格 JSON** 数组。
   5. 校验每题（含 `word` 须在本批词表内且不重复）并批量写入题库；更新 `questionsCreated`、`lastWordAfter`；成功时更新任务 `lastWord`。
...
- 题目题干为含目标词的完整英文例句（禁止挖空）、选项中文词义，按单词字母序且密推进游标续写；执行历史可查。
